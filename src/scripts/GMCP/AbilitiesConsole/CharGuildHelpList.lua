function CharGuildHelpList()
    ...
    for k in pairs(guild_help_list) do table.insert(tkeys, k) end
    -- sort the keys
    table.sort(tkeys)
    -- use the keys to retrieve the values in the sorted order
    for _, k in ipairs(tkeys) do
        -- function
        table.sort(guild_help_list[k], function(v1, v2) return v1 < v2 end)
        ...
        for k2, v2 in pairs(guild_help_list[k]) do
            ...
            cechoLink("GUI.AbilitiesConsole", "<magenta>?<reset>",
                      [[send("ghelp ]] .. v2 .. [[", false)]], "Help", true)
            echo("GUI.AbilitiesConsole", string.rep(" ", 15 - string.len(v2)))

            if count % 2 == 0 then echo("GUI.AbilitiesConsole", "\n") end
        end
        if count % 2 ~= 0 then cecho("GUI.AbilitiesConsole", "\n") end
    end
end
